...
`User` 테이블을 `TRUNCATE` 한 뒤에는:

1. **회원가입**은 백엔드 DB에 새 유저를 만들므로, **회원가입** 페이지에서 새로 가입하면 됩니다.
2. 이전에 **브라우저 localStorage만으로 가입했던 경우**에는, 로그인/방 생성 시 "세션 만료"처럼 나올 수 있습니다. 이때는 **해당 사이트의 저장된 데이터 삭제**(개발자 도구 → Application → Local Storage → 해당 오리진 삭제) 후, 다시 **회원가입**하면 됩니다.
